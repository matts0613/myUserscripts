        ...
        $('article ._97aPb').each(function(){
            ...
            if(!$(this).attr('data-snig')){
                ...
                $(this).on('click','.SNKMS_IG_DW_MAIN',function(e){
                    GM_setValue('mVCompleted',false);
                    GM_setValue('username',$(this).parent().attr('data-username'));
                    GM_setValue('postpath',$(this).parent().parent().find("div.eo2As a.c-Yi7").attr("href"));
                            ...
                            if(!GM_getValue('mVCompleted')){
                                ...
                                if(element_videos && element_videos.attr('src')){
                                    ...
                                    if(element_videos.attr('src').match(/^blob:/ig)){
                                        ...
                                        GM_xmlhttpRequest({
                                            method: "GET",
                                            url: "https://www.instagram.com"+GM_getValue('postpath'),
                                            onload: function(response) {
                                                $('.IG_SN_DIG .IG_SN_DIG_MAIN #loadingText').remove();
                                                let regVD = new RegExp(`,"video_url":"(.*?)"`,"sg");
                                                let regVD2 = new RegExp(`,"video_url":"(.*?)"`,"s");
                                                let regIM = new RegExp(`,"display_url":"(.*?)"`,"sg");
                                                let regIM2 = new RegExp(`,"display_url":"(.*?)"`,"s");
                                                let tot = response.responseText.match(regVD).length;

                                                while(vdI <= tot){
                                                    let vd = response.responseText.match(regVD)[vdI-1].match(regVD2)[1];
                                                    let im = response.responseText.match(regIM)[vdI].match(regIM2)[1];
                                                    let vd_url = decodeURIComponent(JSON.parse('"'+vd+'"'));
                                                    let im_url = decodeURIComponent(JSON.parse('"'+im+'"'));

                                                    $('.IG_SN_DIG .IG_SN_DIG_MAIN').append('<a data-type="mp4" data-globalIndex="'+vdI+'" style="'+style+'" target="_blank" href="'+vd_url+'&dl=1"><img width="100" src="'+im_url+'" /><br/>- Blob Video '+vdI+' -</a>');
                                                    GM_setValue('dw_delay',false);
                                                    vdI++;
                                                }
                                            }
                                        });
                                        GM_setValue('mVCompleted',true);
                                    }
